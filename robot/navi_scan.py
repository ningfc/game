...
def update_drone_position():
    global drone_pos, spiral, path
    
    # 更新螺旋参数
    spiral['angle'] += spiral['speed']
    spiral['radius'] -= spiral['shrink_rate']  # 半径逐渐减小
    
    # 计算新位置
    drone_pos['x'] = spiral['center_x'] + math.cos(math.radians(spiral['angle'])) * spiral['radius']
    drone_pos['y'] = spiral['center_y'] + math.sin(math.radians(spiral['angle'])) * spiral['radius']
    
    # 记录路径
    path.append((drone_pos['x'], drone_pos['y']))
    if len(path) > 2000:  # 路径长度限制
        path.pop(0)
...
def reset_scan():
    """重置扫描状态"""
    global path
    spiral['radius'] = START_RADIUS  # 重置到起始半径
    spiral['angle'] = 0
    path.clear()
    # 将无人机放置在外圈起始位置
    ...
    while running:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_SPACE:  # 空格键重置
                    trees = generate_forest()
                    reset_scan()
                    scan_coverage = ScanCoverage(WIDTH, HEIGHT, SCAN_SIZE, cell_size=10)
